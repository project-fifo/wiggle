...
%%%
%%% @end
%%% Created : 24 Apr 2012 by Heinz N. Gies <>
%%%-------------------------------------------------------------------
-module(wiggle_keymanager).

-behaviour(gen_server).

%% API
-export([start_link/0, create_key/1]).
...
create_key(ID) ->
    gen_server:cast(?SERVER, {create_key, ID}).
...
handle_cast({create_key, ID}, State) ->
    ...
    {ok, Pub} = file:read_file(PubKeyFile),
    file:delete(PubKeyFile),
    wiggle_storage:set_keys(ID, Priv, Pub),
    {noreply, State};
