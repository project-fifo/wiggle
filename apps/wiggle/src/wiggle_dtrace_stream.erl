...
websocket_init(_Any, Req, []) ->
    ...
    {#state{token = Token, scope_perms = SPerms}, Req3} =
        wiggle_handler:get_token(#dstate{}, Req2),
    case Token of
        undefined ->
                ...
                true ->
                    case ls_dtrace:get(ID) of
                        {ok, Obj} ->
                            ...
                            {ok, Req3, #dstate{id = ID, config = ft_dtrace:config(Obj),
...
websocket_handle({Type, <<>>}, Req, State = #dstate{type = Type}) ->
...
websocket_info({tcp, _Port, Data}, Req, State  = #dstate{encoder = Enc, type = Type}) ->
